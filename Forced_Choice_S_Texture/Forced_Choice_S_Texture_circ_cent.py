...
class Patch():
    def __init__(self, center, size, color, back_color,pos):
        ...
        if (pos == up or pos == down):
            print("estoy arriba o abajo")
            self.size_x = int(np.sqrt(2)*size)
            self.size_y = int(size)
        else:
            print("estoy izquierda o derecha")
            ...
            for j in range(int(self.size_y)+1):
                ...
                if self.pos == up:
                    ...
                    if i != i_center:
                        ...
                        angulo = np.pi/2
                    if np.abs(i-i_center) <= np.abs (j-j_center):
                        ...
                        array_circular[i,j,:] = self.back_color
    ...
    for i in range(8) :
        for j in range(Nd):
            ...
            for k in range(Nr):
                ...
                Paux.append(   Patch( position[pos] , size,  rgbaux, rgb_fondo, pos      ) ) # In this case I create a random position for each Trial
        random.shuffle(Paux)
...
def main(start):
    ...
    Nd=15 # Number of bins where you will mesaure  15
    ...
    exp=0.8 # Time of exposition of the Stimuli
