...
def game_intro():
    ...
    start= pygame.image.load("experiment/Images/Startscene.png")
    screen.blit(pygame.transform.scale(start,(w,h)), (0,0))
     
    k = float(inputbox.ask(screen,'Spatial frecuency'))
    screen.blit(pygame.transform.scale(start,(w,h)), (0,0))
    ...
    pygame.display.flip()
    out =  open(basepath+"Subjects/"+Name+"/"+Name+str(i+1)+".dat","w")
    
    out.write("Subject:"+Name+" \n")
    out.write("Session Nr:"+str(i+1)+"\n")
    out.write("Spatial frequency: " + str(k) + "\n")
