...
x_values = np.abs(data_dict[1.5]['[0, 1, 1]']['amplitudes'])

y_values = np.array(data_dict[1.5]['[0, 1, 1]']['results'])

n = 7

# Create intervals
intervals = np.linspace(min(x_values), max(x_values), n + 1)

# Create a DataFrame to store the intervals and associated correct answer fractions
df = pd.DataFrame(index=range(len(intervals)-1), columns=['Interval Start', 'Interval End', 'Correct Answer Fraction'])
...
for i in range(len(intervals)-1):
    ...
    mask = np.where(np.logical_and(x_values >= start_interval, x_values < end_interval))[0]

    
    # Calculate the fraction of correct answers in the interval
    fraction_correct = np.sum(y_values[mask]) / len(mask)
    
    # Populate the DataFrame
    df.loc[i] = [start_interval, end_interval, fraction_correct]
...
bar_centers = (df['Interval Start'] + df['Interval End']) / 2
plt.bar(df['Interval Start'], df['Correct Answer Fraction'], width=np.diff(intervals)[0], color = '#D81159', edgecolor='black', align='edge')

# Customize the plot
plt.xlabel('Amplitudes', fontsize = 16)
plt.ylabel('Fracción de respuestas correctas', fontsize = 16)
#plt.title('Correct Answer Fraction in Intervals')
plt.xticks(intervals, fontsize=14)
plt.yticks(fontsize=14)
#plt.axvline(x = 0.00576)
